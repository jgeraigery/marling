...
def env_reset(env, behavior_policies, custom_reset_config, staggered_reset=False):
    ...
    # print(custom_reset_config)

    # Random actions for a random number of steps if doing staggered resets
    # if staggered_reset:  
    ...
    #                 if agent == 'agent_0':  
    #                     agent_acts.append(random.randint(0, 18))  
    #                 else:  
    #                     agent_acts.append(19)  
    
    #             actions[agent] = {'action': np.array(agent_acts)}
    ...
    #             num_steps = random.randint(0, 10)  # Update num_steps with a new random value  
    #             s = 0  # Reset the step counter
...
def rollout_func(
    ...
    **kwargs
    ...
    step_data = env_reset(env,behavior_policies,custom_reset_config, staggered_reset=not eval)

    step = 0
    step_data_list = []
    results = []

    # Keep track of the average j for each player separately
    action_log_average = [0 for _ in range(env.num_players["agent_0"])]
    ...
    while step <= rollout_length:
        ...
        for agent_id, (policy_id, policy) in behavior_policies.items():
            ...
            # If the policy is not built_in, then the it should not output the "built-in" action, i.e. 19
            if "built_in" not in policy_id:
                assert 19 not in policy_outputs[agent_id][EpisodeKey.ACTION]
