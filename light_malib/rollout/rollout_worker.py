...
class RolloutWorker:
    ...
    # (Re-)Initialize the environments of the rollout workers
    def initialize_envs(self, envs_cfg):
        # First close the previous environments if there is any
        for env_id, env in self.envs.items():
            env.close()
        
        # Reinitialize using the given env config
        self.envs = make_envs(self.id, self.seed, envs_cfg)

    # Reset the rollout length
    def reset_rollout_length(self, new_rollout_length):

        # Reset both non-eval and eval rollout lengths
        self.cfg.rollout_length = new_rollout_length
        self.cfg.eval_rollout_length = new_rollout_length
    ...
    def rollout(self, rollout_desc: RolloutDesc, eval=False, rollout_epoch=None):
        ...
        # NOTE: This is messing with the ordering of the teams 
