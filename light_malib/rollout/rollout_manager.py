...
class RolloutManager:
    def __init__(self, id, cfg, agents: Agents):
        ...
        # Total Self-Play Global Step (Over all of the scenarios so far, important for self-play schemes, e.g. in TiZero)
        self.self_play_global_step = 0
    ...
    # Function to initialize/re-initialize the environment of the rollout workers
    def initialize_workers_env(self, scenario_config):
        # Loop through the rollout workers and initialize their environments
        init_env_futures = []  
        for worker in self.rollout_workers:  
            init_env_futures.append(worker.initialize_envs.remote([scenario_config["envs"]]))

        ray.get(init_env_futures)

    # Reset rollout length
    def reset_rollout_length(self, new_rollout_length):

        # Reset the rollout_length in the current cfg for rollout_manager
        self.cfg.worker.rollout_length = new_rollout_length

        # Reset rollout length for the workers
        reset_rl_len_futures = []
        for worker in self.rollout_workers:
            reset_rl_len_futures.append(worker.reset_rollout_length.remote(new_rollout_length))

        ray.get(reset_rl_len_futures)
    ...
    def rollout(self, rollout_desc: RolloutDesc):
        ...
        if rollout_desc.sync:
            global_rollout_steps = self.sync_rollout(rollout_desc)
        else:
            global_rollout_steps = self.async_rollout(rollout_desc)
    ...
    def sync_rollout(self, rollout_desc: RolloutDesc):
        ...
        try:
            ...
            while True:
                ...
                Logger.info(f'Ended Rollout {rollout_epoch}')
                ...
                if self.cfg.max_global_rollout_steps != -1 and self.self_play_global_step >= self.cfg.max_global_rollout_steps:
                    Logger.warning(f"Reached {self.self_play_global_step} >= {self.cfg.max_global_rollout_steps} max global rollout steps, stopping rollout")
                    break
                ...
                rollout_metrics_mean = self.rollout_metrics.get_means(
                    metric_names=["reward", "win", "win_against_recent"]
                )
                ...
                Logger.info(f'Calling training step for epoch {rollout_epoch}')
        ...
        return self.self_play_global_step
    ...
    def get_global_step(self,rollout_epoch, self_play=False):
        # Return global step for the current scenario only and not the whole self-play
        if not self_play:
            global_step = rollout_epoch * self.batch_size * self.cfg.worker.rollout_length
        # Return the number of steps from the beginning of self-play over all scenarios so far (i.e. cumulative)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            )
        else:
            self.self_play_global_step +=  self.batch_size * self.cfg.worker.rollout_length
            global_step = self.self_play_global_step
            
        return global_step
    
    def log_to_tensorboard(self, results, timer_results, rollout_epoch, main_tag="Rollout"):
        # log to tensorboard, etc...
        global_step=self.get_global_step(rollout_epoch, self_play=True)

        print(f'Logging results to TensorBoard in step: {global_step}')
        # tag = "{}/{}/{}/".format(
        #     main_tag, self.rollout_desc.agent_id, self.rollout_desc.policy_id
        # )
        tag = "{}/{}/".format(
            main_tag, self.rollout_desc.agent_id
        )
        ray.get(
    ...
    def reduce_rollout_results(self, rollout_results):
        ...
        for rollout_result in rollout_results:
            for _result in rollout_result["results"]:
                ...
                main_agent_policy = _result["policy_ids"]["agent_0"]
                opponent_policy = _result["policy_ids"]["agent_1"]

                # Checking if we have played against the most recent version of our policy
                if "-" in opponent_policy:
                    main_agent_policy_nr = int(main_agent_policy.split("-")[-1])
                    opp_policy_nr = int(opponent_policy.split("-")[-1])

                    if opp_policy_nr == main_agent_policy_nr - 1:
                        
                        results["win_against_recent"].append(_result["stats"][main_agent_id]["win"])
                

        for k, v in results.items():
            # if k == "win":
            #     print("WINRATE")
            #     print("without mean: ", v)
            #     print("With mean: ", np.mean(v))
            # if k == "reward":
            #     print("REWARD")
            #     print("With mean: ", np.mean(v))
            #     print("without mean: ", v)
