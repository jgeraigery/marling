...
def tartrl_obs_deal(obs, n_players_per_team):
    ...
    # Remove the goalkeeper from the obs dictionary
    # n_players_per_team = n_players_per_team - 1
    # obs["left_team"] = obs["left_team"][1:]
    # obs["left_team_direction"] = obs["left_team_direction"][1:]
    # obs["left_team_tired_factor"] = obs["left_team_tired_factor"][1:]
    # obs["left_team_yellow_card"] = obs["left_team_yellow_card"][1:]
    # obs["left_team_active"] = obs["left_team_active"][1:]

    # obs["right_team"] = obs["right_team"][1:]
    # obs["right_team_direction"] = obs["right_team_direction"][1:]
    # obs["right_team_tired_factor"] = obs["right_team_tired_factor"][1:]
    # obs["right_team_yellow_card"] = obs["right_team_yellow_card"][1:]
    # obs["right_team_active"] = obs["right_team_active"][1:]

    # obs["ball_owned_player"] = obs["ball_owned_player"] - 1
    # obs["active"] = obs["active"] - 1
    ...
    left_offside = np.zeros(n_players_per_team)
    ...
    left_team = np.concatenate([
        left_position, # 11 * 2
        new_left_direction, # 11 * 2
        left_tired_factor, # 11
        left_yellow_card, # 11
        left_red_card, # 11
        left_offside, # 11
    ]).astype(np.float64)
    ...
    right_offside = np.zeros(n_players_per_team)
    ...
    # active 18
    sticky_actions = obs["sticky_actions"][:10] # 10
    active_position = obs["left_team"][obs["active"]] # 2
    active_direction = obs["left_team_direction"][obs["active"]] # 2
    active_tired_factor = left_tired_factor[obs["active"]] # 1
    active_yellow_card = left_yellow_card[obs["active"]] # 1
    active_red_card = left_red_card[obs["active"]] # 1
    active_offside = left_offside[obs["active"]]  # 1
    ...
    relative_info = np.concatenate([
        ...
        [distance2ball], # 1
        relative_left_position, # 11 * 2
        distance2left, # 11
        relative_right_position, # 11 * 2
        distance2right, # 11
    ]).astype(np.float64)
    ...
    ball_owned_player = np.zeros((n_players_per_team * 2) + 1)
    if obs["ball_owned_team"] == 1:     # 对手
        ball_owned_player[n_players_per_team + obs['ball_owned_player']] = 1.0
    ...
    if obs["game_mode"] == 0:
        ...
        for enemy_pos in obs["right_team"][:]:
    ...
    # Here, if the player is very far from the ball, the best direction to the ball is found and it is
    # set as the only allowed action.
    ...
    ball_pos_offset = 0.02
    no_ball_pos_offset = 0.01
