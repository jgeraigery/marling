...
'''
This is a modified version of the original scenario_builder.py file from the gfootball environment.
Add this file to the gfootball/env directory to use it in the environment. 
This version only adds slight variation to the initialization of non goalkeepers.
'''
...
class Scenario(object):
  ...
  def AddPlayer(self, x, y, role, lazy=False, controllable=True):
    ...
    """
    # Add slight variation to the initialization of non goalkeepers.
    if role != Role.e_PlayerRole_GK:
      x += random.uniform(-0.10, 0.10)
      y += random.uniform(-0.10, 0.10)
      # Check for out of bounds
      x = max(-2.0, min(1.0, x))
      y = max(-1.42, min(0.42, y))
  ...
  def SetBallPosition(self, ball_x, ball_y):
    # Add slight variation to the ball position.
    ball_pos_x = ball_x + random.uniform(-0.10, 0.10)
    ball_pos_y = ball_y + random.uniform(-0.10, 0.10)

    # Check for out of bounds
    ball_pos_x = max(-1.0, min(1.0, ball_pos_x))
    ball_pos_y = max(-0.42, min(0.42, ball_pos_y))

    self._scenario_cfg.ball_position[0] = ball_pos_x
    self._scenario_cfg.ball_position[1] = ball_pos_y
