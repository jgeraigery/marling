...
# Slightly modified football_env_core.py so that it handles no-goalie scenarios and proper window event handling for Windows
# This script should be copied in gfootball/env/ to overwrite the original file for the changes to take effect.
...
import numpy as np
import six.moves.cPickle # type: ignore
...
class FootballEnvCore(object):
  ...
  def step(self, action, extra_data={}):
    ...
    # For Windows event handling
    if sys.platform == 'win32':
      import win32gui
      # Pump all of the waiting messages, necessary for Windows rendering to allow click and dragging the window.
    ...
    # Reset the environment if the goalie has the ball and the goalie is not supposed to be in the play
    # e.g. 3v3 but 4 players in each team
    if ("3_vs_3" in self._config['level'] and len(self._observation['left_team']) == 4 and len(self._observation['right_team']) == 4) \
    or ("5_vs_5" in self._config['level'] and len(self._observation['left_team']) == 6 and len(self._observation['right_team']) == 6):
      if self._observation['game_mode'] == int(
            libgame.e_GameMode.e_GameMode_GoalKick) or self._observation['game_mode'] == int(
              libgame.e_GameMode.e_GameMode_FreeKick) or self._observation['game_mode'] == int(
                libgame.e_GameMode.e_GameMode_Penalty):
        self._env.state = GameState.game_done
      
      # If the goalie has the ball in other situations
      elif self._observation['ball_owned_player'] == 0:
        self._env.state = GameState.game_done
    ...
    # Finish the episode if a red card is given to any player
    if (False in self._observation['left_team_active']) or (False in self._observation['right_team_active']):
      self._env.state = GameState.game_done
