...
class GRFootballEnv(BaseEnv):
    def __init__(self, id, seed, cfg):
        ...
        if 'academy' in scenario_name:
            ...
            self.rewarder = Rewarder_basic(self.cfg.reward_config, num_players_per_team=self.num_players['agent_0'])
    ...
    def reset(self, custom_reset_config):
        ...
        elif '3_vs_3' in self.cfg["scenario_config"]['env_name']:
            self.states = [State(n_player=3) for i in range(self.num_players_total)]
        elif '10_vs_10' in self.cfg["scenario_config"]['env_name']:
            self.states = [State(n_player=10) for i in range(self.num_players_total)]
        else:
            # Logger.info(f'Considering 11 players for each State object')
    ...
    def get_reward(self, rewards):
        self.rewarder.reset_reward_state()
        team_left_states = self.states[: len(self.states) // 2]
        team_right_states = self.states[len(self.states) // 2:]
        rewards = np.array(
            [
            [self.rewarder.calc_reward(reward, state, team_left_states, team=0)]
            if idx < len(rewards) / 2
            else [self.rewarder.calc_reward(reward, state, team_right_states, team=1)]
            for idx, (reward, state) in enumerate(zip(rewards, self.states))
            ],
            dtype=float,
        )

        # Assert that the rewards are zero-sum
        assert np.allclose(np.sum(rewards, axis=0), 0), f"Rewards are not zero-sum. Calculated rewards: {rewards}"
