...
def recurrent_generator(data, num_mini_batch, rnn_data_chunk_length, device):
    ...
    for k in batch:
        ...
        if k == "global_state":  
            batch[k] = batch[k].repeat(1, 1, n_agent, 1)  
        batch[k] = _cast(batch[k])

    # jh: special optimization
    # TODO: Check if this is correct with an LSTM, this basically flattens and passes in a whole sequence into the LSTM, instead of chunks
    if num_mini_batch == 1:
        for k in batch:
